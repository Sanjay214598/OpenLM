...
function ViewOrderListController($scope, $rootScope, Orders, messageService, $location, $routeParams, $window, supplylines, programs, schedules, years, ReportPeriodsByScheduleAndYear, ReportProgramSchedules,OrdersAudit) {
  ...
  function successFunc(data)
    {
      console.log(data);
	  for(var i = 0 ; i < data.orderFileTemplate.length; i++){
		var rnr = data.orderFileTemplate[i];
		var dt = new Date(rnr.createdDate);
		dt = dt.toString().substr(4, dt.toString().indexOf("GMT"));
		$scope.trail.push({orderNumber: $rootScope.trails.id, rnrNo: $rootScope.trails.rnr.id, program: $rootScope.trails.rnr.programName, stringCreatedDate: dt, status: rnr.status});
	  }
	  $scope.trail.push({orderNumber: $rootScope.trails.id, rnrNo: $rootScope.trails.rnr.id, program: $rootScope.trails.rnr.programName, stringCreatedDate: $rootScope.trails.stringCreatedDate, status: $rootScope.trails.status});
	  
    }
  if($rootScope.trails != undefined){
	OrdersAudit.get({id: $rootScope.trails.id}, successFunc, function (data) {});
  }
  ...
  function refreshGrid() {
	...
	Orders.get({page: $scope.currentPage, supplyDepot: 2, program: $scope.program, period: "1,2" }, function (data) {
  ...
  $scope.gridOptions = { data: 'orders',
    ...
    columnDefs: [
	  ...
	  {displayName: "Audit",cellTemplate: "<div class='ngCellText'><span ng-show=\"row.entity.productsOrdered\"><a  ng-click='loadById(row)'>View Audit Trail</a>  </span>" +
        "</div>", width: 180}
    ]
  };
  $scope.loadById = function(row) {  
   window.console && console.log(row.entity);
   $rootScope.trails = row.entity;
   $window.location.href= '/public/pages/logistics/fulfillment/index.html#/'+ row.entity.id + '/audit';
};
  //var trails = {orderNumber: 1, rnrNo: 1, program: "Malaria", stringCreatedDate: "21-12-2018", status: "Status"};
  $scope.trailGridOptions = { data: 'trail',
    showFooter: false,
    showColumnMenu: false,
    showFilter: false,
    enableColumnResize: true,
    ...
    columnDefs: [
      ...
      {field: 'rnrNo', displayName: "Requisition Id"},
      {field: 'stringCreatedDate', displayName: messageService.get("label.order.date.time")},
      {field: 'status', displayName: messageService.get("label.order.status")}
    ]
  };
